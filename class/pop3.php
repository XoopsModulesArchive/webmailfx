...
class POP3{
	...
	function POP3($hostname,$user,$password,$apop,$DEBUG) {
	...
	function POP3Command($command, $result="") {
		...
		$result = fread($this->connection, 100);
	...
	function Stats($msg=""){
		...
		if ($msg == "") :
		$result = $this-> POP3Command ("NOOP");		
		$result = $this->POP3Command("STAT");
		else :
		$result = $this->POP3Command("LIST $msg");
		endif;
		$p = explode(" ", $result);
		$stat["message"] = $p[1];
		$stat["size"] = $p[2];
	...
	function GetMessageID($message="") {
		if ($message) :
		$result = $this->POP3Command("UIDL $message");
		$id = explode (" ", $result);
